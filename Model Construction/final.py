...
def plot_confusion_matrix(model, n_epochs, n_batch_size, n_neurons, cm, classes, normalize=False, title='Confusion matrix', cmap=plt.cm.get_cmap('Blues')):
    ...
    plt.savefig(r'D:\Tugas Akhir\Final Dev-4\Results Data\%s_CONFUSION_MATRIX_CATEGORICAL_%s_E%s_B%s_N%s.png' %(date, model, n_epochs, n_batch_size, n_neurons))
...
def model_fit(x, y, x_test, y_test, n_epochs, n_batch_size, n_neurons, model_arch):
    if model_arch == "RNN" :
        model = Sequential()
    ...
    data.to_csv(r'D:\Tugas Akhir\Final Dev-4\Results Data\%s_DATA_FINAL__CATEGORICAL_%s_E%s_B%s_N%s.csv' %(date, model_arch, n_epochs, n_batch_size, n_neurons))
    ...
    plt.savefig(r'D:\Tugas Akhir\Final Dev-4\Results Data\%s_ACC__CATEGORICAL_%s_E%s_B%s_N%s.png' %(date, model_arch, n_epochs, n_batch_size, n_neurons))
    ...
    plt.xlabel('epoch')
    plt.legend(['train', 'test'], loc='upper left')
    plt.savefig(r'D:\Tugas Akhir\Final Dev-4\Results Data\%s_LOSS__CATEGORICAL_%s_E%s_B%s_N%s.png' %(date, model_arch, n_epochs, n_batch_size, n_neurons))
...
def experiment():
    ...
    neuron = 23
    model = "RNN"
    model_fit(x, y, x_t, y_t, epochs, batch_size, neuron, model)
